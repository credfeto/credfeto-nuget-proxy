...
internal static partial class JsonIndexTransformerBaseLoggingExtensions
{
    ...
    public static partial void UpstreamFailed(this ILogger logger, Uri upstream, HttpStatusCode statusCode);

    [LoggerMessage(LogLevel.Information, EventId = 2, Message = "Retrieved JSON from {upstream} Received Http {statusCode} Length: {length}")]
    public static partial void UpstreamOk(this ILogger logger, Uri upstream, HttpStatusCode statusCode, int length);
}